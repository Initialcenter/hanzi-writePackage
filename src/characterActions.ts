...
export const animateCharacter = (
  ...
  highlightColor?: ColorObject | null,
): GenericMutation[] => {
  ...
  character.strokes.forEach((stroke, i) => {
    ...
    // TIPS: 给每一笔添加一个高亮的动画
    if(highlightColor){
      mutations = mutations.concat(highlightStroke(stroke, highlightColor, speed));
      mutations = mutations.concat(animateStroke(charName, stroke, 99999));
    }else{
