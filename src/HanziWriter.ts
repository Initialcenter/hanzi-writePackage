...
export default class HanziWriter {
  ...
  animateCharacter(
    ...
    const runAnimation = (highlightColor?: string) => {
      const colorValues = highlightColor ? colorStringToVals(highlightColor) : undefined;
    ...
    return this._withData(() => runAnimation(options.highlightColor));
  }
  ...
  setDisableDraw(
    ...
    this._disableDraw = options.disabled;
    // console.log(this._disableDraw,'禁用绘制')
  }

  // 数据类型Point{x: 78, y: 142} PointType: 0:下笔 1:移动 2:抬笔
  // 智能笔入口
  penOpen(Point, PointType){
    // 下笔
    if(PointType === 0){
      if (this._quiz && !this._disableDraw) {
        this._quiz.startUserStroke(Point);
      }
    }
    // 移动
    if(PointType === 1){
      if (this._quiz && !this._disableDraw) {
        this._quiz.continueUserStroke(Point);
      }
    }
    // 抬笔
    if(PointType === 2){
